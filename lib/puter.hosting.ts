...
import { createHostingSlug, HOSTING_CONFIG_KEY, 
    isHostedUrl, fetchBlobFromUrl, 
    imageUrlToPngBlob, getImageExtension,
    getHostedUrl} from "./utils";
...
export const uploadImageToHosting = async ({ hosting, url, projectId, label}:
    StoreHostedImageParams): Promise<HostedAsset | null> => {
        ...
        try {
            ...
            try {
                await puter.fs.mkdir('projects', { createMissingParents: true });
